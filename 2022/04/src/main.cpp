...
int main(int argc, char **argv)
{
    ...
    if (file.is_open())
    {
        ...
        while (getline(file, line))
        {
            ...
            vector<int> elf2_range;
            for (int i = elf2_start_int; i <= elf2_end_int; i++)
            {
                elf2_range.push_back(i);
            }

            set<int> elf1_set(elf1_range.begin(), elf1_range.end());
            set<int> elf2_set(elf2_range.begin(), elf2_range.end());

            vector<int> intersection;
            set_intersection(elf1_set.begin(), elf1_set.end(), elf2_set.begin(), elf2_set.end(), back_inserter(intersection));
